...
bundle_script="$PLTS_HOME/devbin/@bundle"
...
for test_file in "$tests_dir"/*.pl; do
  (( total_count += 1 ))
  ...
  swipl -q -f "$test_file" -g main -g halt 1>"$exp_out" 2>"$exp_err" \
  	|| true

  # Compile to JavaScript
  jsfile="$tmpdir/${name}.js"
  "$bundle_script" "$test_file" >"$jsfile"
